...
def part2(input: list[str]):
  ...
  def matchRegion(r, c):
    ...
    if c > 0 and input[r][c] == input[r][c-1]:
      ...
      regionsMatched.append(plotToRegion[(r,c-1)])
    if c > 0 and r > 0 and input[r][c] == input[r-1][c-1]:
      sidesMatched.append('lc')
    if c < len(input[0]) - 1 and r > 0 and input[r][c] == input[r-1][c+1]:
    ...
    for c in range(len(input[r])):
      ...
      if region is not None:
        ...
        if sidesMatched == ['t', 'l'] or sidesMatched == ['t', 'l', 'lc']:
          regions[region][2] -= 2
        elif sidesMatched == ['t', 'lc'] or sidesMatched == ['l', 'lc'] or sidesMatched == ['t', 'rc']:
          regions[region][2] += 2
