...
def checkPair1(r1: int, c1: int, r2: int, c2: int, height: int, width: int) -> set:
    ...
    while True:
        r += dr
        c += dc
        if not (r >= 0 and c >= 0 and r < height and c < width):
            break
        ...
        d2 = (r2 - r, c2 - c)
        # print(r, c, d1, d2)
        if (d1[0] * 2 == d2[0] and d1[1] * 2 == d2[1]) or (
            d1[0] == d2[0] * 2 and d1[1] == d2[1] * 2
        ):
            antinodes.add((r, c))
...
def mapOut1(data: list[str]) -> set[tuple[int, int]]:
...
def mapOut2(data: list[str]) -> set[tuple[int, int]]:
    antennas: defaultdict[str, list[tuple[int, int]]] = defaultdict(list)
    antinodes: set[tuple[int, int]] = set()
    height = len(data)
    width = len(data[0])
